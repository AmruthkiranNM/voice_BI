  ...
  return (
    <div className="panel-card">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-data uppercase tracking-wide text-zinc-500">Answer</h3>
        {isSupported && (
          <button
            type="button"
            onClick={() => isSpeaking ? stop() : speak(insight)}
            className="text-xs text-zinc-500 hover:text-[#9C4A2A] transition-colors"
          >
            {isSpeaking ? 'Stop' : 'Listen'}
          </button>
        )}
      </div>
      <RichText text={insight} className="text-sm sm:text-base text-ink" />
    </div>
  );
}
